...
h1, h2, h3, h4, h5, h6, p, label, textarea, input, button, .stMarkdown, .stText {
    font-family: 'Noto Sans KR', sans-serif !important;
    color: #333333 !important;
}

/* 2. Streamlit Expander 아이콘 강제 보호 */
    ...
    with t1:
        ...
        with st.expander("📂 전체 업무 리스트 확인 (주기별 모아보기)"):
            ...
            if res_defs.success and not res_defs.data.empty:
                ...
                # 주기별로 탭 생성
                cycles = df_defs["cycle_type"].unique()
                if len(cycles) > 0:
                    tabs = st.tabs([c for c in cycles])
                    for i, cycle in enumerate(cycles):
                        with tabs[i]:
                            subset = df_defs[df_defs["cycle_type"] == cycle]
                            for _, row in subset.iterrows():
                                is_done = str(row['id']) in today_done_ids
                                status_icon = "✅" if is_done else "⬜"
                                status_text = "(완료됨)" if is_done else "(미완료)"
                                st.markdown(f"""
                                    <div style="padding:10px; border-bottom:1px solid #eee;">
                                        {status_icon} <b>{row['task_name']}</b> <small style='color:#888;'>{status_text}</small>
