...
# [수정됨] CSS: 폰트 적용 범위를 'span', 'div'에서 제외하여 아이콘 깨짐 방지
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;500;700&display=swap');

/* 1. 명확한 텍스트 태그에만 폰트 적용 (아이콘은 보통 span이나 i 태그를 씀) */
...
[data-testid="stExpanderToggleIcon"] {
    font-family: "Material Icons" !important; /* 원래 아이콘 폰트 유지 */
}
...
# [추가] 안전한 쿠키 조회 함수 (에러 방지용)
def safe_get_cookie(key):
    if cookies is None:
        return None
    try:
        # 쿠키 매니저가 준비되지 않았을 때 접근하면 에러가 발생하므로 try-except 필수
