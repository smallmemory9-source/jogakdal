...
# 시트 탭 이름 매핑 (구글 시트의 탭 이름과 정확히 일치해야 합니다)
SHEET_NAMES = {
    "users": "users",
    "posts": "posts",
    "comments": "comments",
...
def load(key):
    """구글 시트에서 데이터를 불러옵니다."""
    try:
        # ttl=0 : 캐시를 쓰지 않고 항상 최신 데이터를 가져옴
        df = conn.read(worksheet=SHEET_NAMES[key], ttl=0)
        return df
    except Exception as e:
        # 탭이 없거나 에러가 나면 빈 데이터프레임 반환
        # st.error(f"데이터 로드 오류 ({key}): {e}") 
        return pd.DataFrame()
...
def init_db():
    ...
    # 1. users 시트가 비어있거나, admin 계정이 없으면 초기화
    ...
    elif "admin" not in users["username"].values:
        # 컬럼은 있는데 admin이 없으면 추가
        admin_pw = hash_password("1234")
        new_row = pd.DataFrame([{"username": "admin", "password": admin_pw, "name": "사장님", "role": "Manager"}])
        save("users", pd.concat([users, new_row], ignore_index=True))

    # 다른 테이블들도 컬럼이 잘 있는지 확인 (최초 1회 로드 시도)
    load("posts")
    load("comments")
    load("routine_def")
    load("routine_log")
...
def is_task_due(start_date_str, cycle_type, interval_val):
    try:
        if pd.isna(start_date_str): return False
...
def get_pending_routines():
    ...
    for _, task in defs.iterrows():
        if is_task_due(task["start_date"], task["cycle_type"], task["interval_val"]):
            # 오늘 완료했는지 확인 (logs가 비어있지 않을 때만 체크)
            if not logs.empty:
                # task_id가 숫자형/문자형 섞일 수 있으므로 문자열로 통일하여 비교
            ...
            with st.expander("⚙️ 반복 업무 추가/삭제 (관리자)"):
                with st.form("add_routine"):
                    ...
                    if st.form_submit_button("업무 추가"):
                        new_id = 1
                        if not defs.empty: new_id = defs["id"].max() + 1
                ...
                for task in due_tasks:
                    ...
                    with st.container():
                        ...
                        if is_done:
                            ...
                            st.success(f"✅ {worker}님 처리 완료")
                        else:
                            if st.button("완료 하기", key=f"do_task_{task['id']}"):
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        if st.session_state.get("show_login_alert", False):
            if pending_list:
                st.toast(f"오늘 할 일이 {len(pending_list)}건 있습니다!", icon="🚨")
                time.sleep(1)
        ...
        if pending_list:
            ...
            <div class="warning-banner">
                🚨 미완료 업무 {len(pending_list)}건! <span style='font-size:0.8em;'>({', '.join(pending_list[:2])}...)</span>
            </div>
            """, unsafe_allow_html=True)
